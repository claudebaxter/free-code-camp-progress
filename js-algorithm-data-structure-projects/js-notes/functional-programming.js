...
const squareList = arr => {
  // Only change code below this line
  return arr
    //filter the array for positive integers 
    .filter(num => num > 0 && num % parseInt(num) === 0)
    //map the values from filter function to the variable squaredIntegers
...
function nonMutatingSort(arr) {
  // Only change code below this line
  //concatenate array taken in as a parameter to a new array
  return [].concat(arr).sort(function(a ,b) {
    //Then use sort()---^^^ method and create function to sort new array in ascending order (return a - b;)
    return a - b;
  });
...
function splitify(str) {
  ...
  let newArr = str.split(/\W/); ///\W\ is equivalent to /[^A-Za-z0-9_]/ to match any non-word character.
...
function urlSlug(title) {
  return title
    .toLowerCase()
    .trim()
    .split(/\s+/) //regex removes spaces
    .join("-"); //joins string with -'s (instead of spaces)
