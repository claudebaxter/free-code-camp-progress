...
// arr.flat(Infinity); would solve this but against the rules

//SOLUTION:

function steamrollArray(arr) {
  //create new empty array (flattenedArray)
  const flattenedArray = [];
  //loop over arr argument contents
  for (let i = 0; i < arr.length; i++) {
...
function binaryAgent(str) {
  ...
  /*using radix (or base) parameter in parseInt, we can convert the binary
  ...
  //we thens imply join the string
...
function truthCheck(collection, pre) {
  ...
  for (let c in collection) {
    //if it has property and value is truthy
...
function addTogether() {
  const [first, second] = arguments; //define expected args
  if (typeof(first) !== "number")
    return undefined; //If first arg isn't a number returns undefined
  if (arguments.length === 1)
    return (second) => addTogether(first, second); //If only one arg is entered, return a function that uses that number and expects another one, to then add to it
  if (typeof(second) !== "number")
    return undefined //returns undefined if 2nd arg isn't a number
  return first + second; //pretty straight forward--returns the sum of both args
}
